...
from snomedct_terminology_server.\
    server.serializers import (ConceptListSerializer,
...
class ListDirectParents(GlobalFilterMixin, ListAPIView):
   ...
   def get_queryset(self):
      ...
      query = """select array(\
 select jsonb_array_elements(parents)->>'concept_id'\
...
The root concept can be accessed through `/terminology/concept/root`.
   ...
   def get_queryset(self):
      ...
      query = """select array(\
 select jsonb_array_elements(children)->>'concept_id'\
 from snomed_denormalized_concept_view_for_current_snapshot\
 where id = %s)"""
      children =  [int(child)
                   for child in
...
class ListDefiningRelationships(GlobalFilterMixin,
                                ListAPIView):
...
In the current version, that is `900000000000011006 |Inferred relationship|`.
   ...
   def get_queryset(self):
      ...
      defining_relationships = Relationship.objects.filter(
          source_id=concept_id, characteristic_type_id = 900000000000011006)
...
class ListAllowableQualifiers(GlobalFilterMixin,
                              ListAPIView):
...
particular jurisdiction or use case.
   ...
   def get_queryset(self):
      ...
      qualifying_relationships = Relationship.objects.filter(
          source_id=concept_id, characteristic_type_id = 900000000000225001)
...
'10362701000001108' as the param to this endpoint. You used: {}""".format(
    type_id))
...
"""
    ...
    query = """SELECT json_object(array_agg(id::text), array_agg(preferred_term)) FROM \
snomed_denormalized_concept_view_for_current_snapshot WHERE id IN %s"""
    if ids:
        ...
        raise APIException(detail="Please provide a list of sctids. You sent: {}".format(request.data))
