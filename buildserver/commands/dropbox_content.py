...
class DropboxData(object):
    ...
    def __init__(self):
        try:
            ...
            self.upstream_metadata = self.client.files_list_folder('/downloads')
    ...
    def file_has_changed(self, file_path):
        ...
        if file_path not in self.extract_file_paths(self.path_keyed_upstream_metadata):
            raise Exception('Inconsistent state; should not occur')

        if file_path not in self.extract_file_paths(self.path_keyed_stored_metadata):
    ...
    def get_upstream_file_paths(self):
        ...
        return (
            entry['path'] for entry in self.path_keyed_upstream_metadata
        )
    ...
    def get_cached_file_paths(self):
        ...
        if not self.stored_metadata:
            self.stored_metadata = self.get_stored_metadata()
        return (
            entry['path'] for entry in self.path_keyed_stored_metadata
        )
    ...
    def save_zip_entry(self, zipfile, zip_entry):
        ...
        full_release_pattern = re.compile(r'(.*)RF2Release(.*)Full/(.*)txt$')

        if zip_entry.endswith('.txt') and full_release_pattern.match(zip_entry):
    ...
    def extract_zips(self):
        ...
        for source_file_path in source_file_paths:
            if source_file_path.endswith('.zip'):
                zf = zipfile.ZipFile(
                    WORKING_FOLDER +
                    source_file_path.replace('downloads/', ''), 'r')
                entries = zf.namelist()
